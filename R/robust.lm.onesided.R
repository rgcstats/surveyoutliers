...
#' @param data A data frame including the variables in formula, and gregwt (generalized regression estimator weight).
...
robust.lm.onesided <- function( formula , data , Q , maxit=100,stop=F){
  if(stop) browser()
  y <- data[,as.character(formula)[2]]
  robust.resid <- (y-median(y))/(1.483*(quantile(y,probs=0.75)-quantile(y,probs=0.25)))
  data$irls.w <- pmin(robust.resid,2) / robust.resid
  ...
  while((iter<=100)&(delta>=1e-6)){
    ...
    data$irls.w <- pmin( y - wfit$fitted.values , Q / (data$gregwt-1) ) / ( y - wfit$fitted.values )
